export const lang = {
  id: {
    NAV: {
      INDEX: 'Home',
      SEARCH: 'Cari',
      ABOUT: 'Tentang Saya',
      FRINEDS: 'Friends',
      PROJECTS: 'Proyek',
      NOTES: 'Catatan',
      NEWSLETTER: 'Newsletter',
      BOOKS: 'Pustaka',
      CONTACT: 'Kontak'
    },
    PAGINATION: {
      PREV: 'Sebelumnya',
      NEXT: 'Selanjutnya'
    },
    LAYOUT: {
      NOTICE_TEXT: 'Jika kamu punya pertanyaan, silakan hubungi aku.',
      PAY_BUTTON: 'Support me',
      NOTICE_BUTTON: 'Kontak',
      COMMENT_TITLE: 'Komentar'
    },
    SEARCH: {
      ONLY_SEARCH: 'Cari tag saja',
      PLACEHOLDER: 'Cari Post',
      NOT_FOUND: 'cari unicorn? mohon maaf ga ada..'
    },
    HERO: {
      HOME: {
        CONTACT_BUTTON: 'Contact',
        CONTACT_BUTTON_DES: 'One Click',
        RSS_BUTTON: 'Subscribe'
      },
      NEWSLETTER: {
        SUBSCRIPTION_HEAD: 'Subscription Channels',
        TG_CHANNEL: 'Telegram Channel',
        RSS_BUTTON: 'Subscribe RSS'
      },
      NOTES: {
        TEXT_FOOTER: 'Dibuat dari Craft.do'
      },
      RSS_BUTTON_DES: 'RSS Reader',
      RSS_BUTTON_COPIED: 'Disalin',
      RSS_BUTTON_DES_COPIED: 'URL RSS disalin'
    },
    CONTACT: {
      TITLE: 'Kontak',
      DESCRIPTION: 'Jika kamu punya pertanyaan, silakan hubungi aku.',
      TG_DESCRIPTION: 'Atau chat langsung via Telegram: ',
      SUCCESS_MESSAGE: 'Pesan terkirim, akan aku balas jika memungkinkan, Insya Allah.',
      FORM_USERNAME: 'Nama Kamu',
      FORM_EMAIL: 'Email/Telegram*',
      FORM_CONTENT: 'Tulis pesan di sini.',
      SEND_BUTTON: 'Kirim',
      FORM_EMAIL_REQUIRED: '*Kamu boleh isi dengan jenis kontak yang lain. ',
      FAILED_MESSAGE: 'Maaf. Gagal mengirim pesan, chat langsung ke Telegram aja ya.'
    },
    FOOTER: {
      COPYRIGHT_START: 'This site is licensed under the ',
      COPYRIGHT_NAME: 'CC BY-SA 4.0 ',
      COPYRIGHT_LINK: 'https://creativecommons.org/licenses/by-sa/4.0/',
      COPYRIGHT_END: '| Powered by Notion'
    },
    ERROR: {
      MESSAGE: 'cari unicorn?',
      TITLE: 'Seperti unicorn, halaman yang kamu cari tidak ada :(',
      HELP_TEXT: 'SIlakan kembali ke halaman awal dan cari. Kalau kamu punya pertanyaan, kontak aku.',
      BACK_TO_HOME: 'Back to Home',
      CRAFTDOCS_ERROR: 'If you see this line, it means your Craft.do configuration page format is wrong.',
      LOADING: '...Loading...',
      TIMEOUT_TEXT: 'Terlalu lama? Klik di sini untuk ke halaman Notion.'
    }
  },
  en: {
    NAV: {
      INDEX: 'Home',
      SEARCH: 'Search',
      ABOUT: 'About',
      FRINEDS: 'Friends',
      PROJECTS: 'Projects',
      NOTES: 'Notes',
      NEWSLETTER: 'Newsletter',
      BOOKS: 'Books',
      CONTACT: 'Contact'
    },
    PAGINATION: {
      PREV: 'Prev',
      NEXT: 'Next'
    },
    LAYOUT: {
      NOTICE_TEXT: 'If you have any questions, please contact me.',
      PAY_BUTTON: 'Support me',
      NOTICE_BUTTON: 'Contact',
      COMMENT_TITLE: 'Comments'
    },
    SEARCH: {
      ONLY_SEARCH: 'Only search tags',
      PLACEHOLDER: 'Search Posts',
      NOT_FOUND: 'No Posts Found'
    },
    HERO: {
      HOME: {
        CONTACT_BUTTON: 'Contact',
        CONTACT_BUTTON_DES: 'One Click',
        RSS_BUTTON: 'Subscribe'
      },
      NEWSLETTER: {
        SUBSCRIPTION_HEAD: 'Subscription Channels',
        TG_CHANNEL: 'Telegram Channel',
        RSS_BUTTON: 'Subscribe RSS'
      },
      NOTES: {
        TEXT_FOOTER: 'Automatically generated from the Craft.do page'
      },
      RSS_BUTTON_DES: 'RSS Reader',
      RSS_BUTTON_COPIED: 'Copied',
      RSS_BUTTON_DES_COPIED: 'Copied RSS URL'
    },
    CONTACT: {
      TITLE: 'Contact',
      DESCRIPTION: 'If you have any questions or suggestions, please contact me.',
      TG_DESCRIPTION: 'Or you can contact me via Telegram: ',
      SUCCESS_MESSAGE: 'Message sent, I will reply to you as soon as possible.',
      FORM_USERNAME: 'Your Name',
      FORM_EMAIL: 'Your Email*',
      FORM_CONTENT: 'Message',
      SEND_BUTTON: 'Send',
      FORM_EMAIL_REQUIRED: '*You can fill in other valid contact methods',
      FAILED_MESSAGE: 'Sorry, sending failed, please contact me directly on Telegram.'
    },
    FOOTER: {
      COPYRIGHT_START: 'This site is licensed under the ',
      COPYRIGHT_NAME: 'CC BY-SA 4.0 ',
      COPYRIGHT_LINK: 'https://creativecommons.org/licenses/by-sa/4.0/',
      COPYRIGHT_END: '| Powered by Notion'
    },
    ERROR: {
      MESSAGE: 'Some errors have occurred',
      TITLE: 'Page Not Found',
      HELP_TEXT: 'Please try to refresh the page, or return to the home page and search, if you have any questions, contact me.',
      BACK_TO_HOME: 'Back to Home',
      CRAFTDOCS_ERROR: 'If you see this line, it means your Craft.do configuration page format is wrong.',
      LOADING: '...Loading...',
      TIMEOUT_TEXT: 'Waiting too long? Click to visit the Notion page directly.'
    }
  },
  ja: {},
  es: {}
}
